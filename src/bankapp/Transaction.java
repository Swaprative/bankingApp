...
public class Transaction extends javax.swing.JDialog {
     ...
     */
    public Transaction(java.awt.Frame parent, boolean modal, Bank bank, Customer customer) {
        super(parent, modal);
        initComponents();
    ...
    private void withdrawalButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_withdrawalButtonActionPerformed
        ...
        if (amountField.getText().isEmpty()) {
            warnings.append("Transaction amount is required.\n");
        } else {
            double amount = 0;
            try {
                amount = Bank.round(Double.parseDouble(amountField.getText()), 2);
                int result = JOptionPane.showConfirmDialog(this, "Transaction Rs. " + String.format("%.2f", amount) + " from the account?\nTransaction Fee: Rs." + String.format("%.2f", bank.getTransactionFee(customer.getAccount().getAccountType())));
                ...
                warnings.append("Transaction amount must be a number.\n");
            }
        }
        if (warnings.length() > 0) {
            JOptionPane.showMessageDialog(this, warnings.toString(), "Transaction Warnings", JOptionPane.WARNING_MESSAGE);
        }
